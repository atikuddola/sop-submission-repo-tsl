...
> See **Section 5 (Appendix)** for the full annotated `patroni.yml`. Change `name`, `restapi`, and `postgresql` IP values per node.

**Per-node differences in patroni.yml:**

| Setting                    | pgnode1         | pgnode2         | pgnode3         |
|----------------------------|-----------------|-----------------|-----------------|
| `name`                     | pgnode1         | pgnode2         | pgnode3         |
| `restapi.connect_address`  | 192.168.122.98  | 192.168.122.147 | 192.168.122.105 |
| `postgresql.connect_address`| 192.168.122.98 | 192.168.122.147 | 192.168.122.105 |
...
### 5.1 Sample patroni.yml (Annotated — pgnode1)

```yaml
# Cluster identity — must match across nodes
scope: postgres
namespace: /db/
name: pgnode1                    # Unique per node: pgnode1, pgnode2, pgnode3

# REST API — HAProxy uses this for health checks
restapi:
    listen: 0.0.0.0:8008         # Listen on all interfaces
    connect_address: 192.168.122.98:8008   # This node's IP — change per node

# etcd DCS connection — same on all nodes
etcd3:
    hosts: 192.168.122.98:2379,192.168.122.147:2379,192.168.122.105:2379

# Bootstrap settings — used only on first cluster init
bootstrap:
    dcs:
        ttl: 30                             # Leader lock TTL in seconds
        loop_wait: 10                       # Patroni loop interval
        retry_timeout: 10                   # DCS operation timeout
        maximum_lag_on_failover: 1048576    # Max replica lag allowed (1MB)
        postgresql:
            use_pg_rewind: true             # Allow rewinding diverged replica WAL

    # initdb is REQUIRED — without it Patroni will never bootstrap
    initdb:
    - encoding: UTF8
    - data-checksums

    # pg_hba rules — written to pg_hba.conf on the primary
    # These are also stored in etcd — use patronictl edit-config to update at runtime
    pg_hba:
    - host replication replicator 192.168.122.98/32 scram-sha-256
    - host replication replicator 192.168.122.147/32 scram-sha-256
    - host replication replicator 192.168.122.105/32 scram-sha-256
...
postgresql:
    listen: 0.0.0.0:5432
    connect_address: 192.168.122.98:5432   # This node's IP — change per node
    data_dir: /data/pgsql/16/data
    bin_dir: /usr/pgsql-16/bin
    authentication:
        replication:
            username: replicator
            password: replicator           # Change in production
        superuser:
            username: postgres
...
# Watchdog — off is fine for most setups; failover still works via etcd
watchdog:
    mode: off
    device: /dev/watchdog
    safety_margin: 5
...
### 5.2 Per-Node Differences Summary
...
| etcd `LISTEN_PEER_URLS` IP    | 192.168.122.98  | 192.168.122.147 | 192.168.122.105 |
| etcd `LISTEN_CLIENT_URLS` IP  | 192.168.122.98  | 192.168.122.147 | 192.168.122.105 |
